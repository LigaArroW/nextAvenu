...
async function encryptString(plainText, token) {
...
const getCaptcha = async (request, response) => {
  try {
    ...
    for (let index = 0; index < numberOfKeys; index++) {
      ...
      const pixelArray = [] as any;
      for (let i = 0; i < pixels.length; i += 4) {
        pixelArray.push(pixels[i + 3]);
      }

      let flipedPixelArray = [] as any;
...
const getVerificationsforAgency = (request, response) => {
  try {
...
export { getCaptcha, verifyCaptcha, getVerificationsforAgency, verifyCaptchaGoogle };
